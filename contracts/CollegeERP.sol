...
contract CollegeERP is Ownable {
    ...
    mapping(string => uint256) public subjectFee;

    event FacultyAdded(address indexed faculty, string name);
    event SubjectAssigned(address indexed faculty, string subject, uint256 fee);
    ...
    function assignSubject(address facultyAddress, string memory subject, uint256 fee) external onlyOwner {
        ...
        emit SubjectAssigned(facultyAddress, subject, fee);
    }
    ...
    function addStudent(address studentAddress, string memory name, string memory branch) external onlyOwner {
        ...
        student.balance = 100; // Initialize with 100 coins
    ...
    function enrollStudent(address studentAddress, string memory subject) external onlyOwner {
        ...
        require(!isEnrolled(studentAddress, subject), "Student already enrolled");
        
        Student storage student = studentDetails[studentAddress];
        uint256 fee = subjectFee[subject];
        require(student.balance >= fee, "Insufficient balance for course fee");
    ...
    function getStudentDetails() external view onlyStudent returns (
        ...
        return (name, studentAddress, branch, balance, subjects, marks);
    }
